    ...
    try {
      ...
      try {
        ...
        if (chat) {
          ...
          if (!('YoutubeLink' in chat)) chat.viewonce = false
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.reply(m.chat, `Limit kamu habis, silahkan beli melalui *${usedPrefix}buy*. atau bisa daftar ke premium melalui owner *${usedPrefix}creator*`, m)
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    owner: '_*Lu Siapa?*_',
    mods: '_*Lu Siapa?*_',
    premium: 'Perintah ini hanya untuk pengguna _*Premium*_ daftar dulu bosku ama owner nya ketik *.creator* untuk lihat nomer ownernya',
    group: 'di grup ini bisa nya',
    private: 'pc aja gua kalo mau make ini! ',
    admin: '_*Lah lu member njirr,,, Mau Ngapain?*_',
    botAdmin: 'Jadikan bot sebagai *Admin* untuk menggunakan perintah ini',
    unreg: 'Silahkan daftar untuk menggunakan fitur ini dengan cara mengetik:\n\n*#daftar nama.umur*\n\nContoh: *#daftar Irwan.17*',
